...
class BookRecommendationEngine:
    ...
    def prepare_clustering(self):
        ...
        features = ['Rating', 'Number of Reviews', 'Price', 'Description_Length', 'Title_Length']
        ...
        kmeans = KMeans(n_clusters=min(8, len(self.df)), random_state=42, n_init=10)
    ...
    def cluster_based_recommendations(self, book_title, n_recommendations=10):
        ...
        try:
            book_cluster = self.df[self.df['Book Name'] == book_title]['Cluster'].iloc[0]
    ...
    def hybrid_recommendations(self, book_title, n_recommendations=10, content_weight=0.6):
        ...
        content_recs['Content_Score'] = content_recs.get('Similarity_Score', 0) * content_weight
        cluster_recs['Cluster_Score'] = (1 - content_weight)
        
        # Merge and calculate final scores
        all_recs = pd.concat([content_recs, cluster_recs]).drop_duplicates(subset=['Book Name'])
        all_recs['Final_Score'] = (all_recs.get('Content_Score', 0) + 
                                  all_recs.get('Cluster_Score', 0) + 
                                  all_recs['Rating'] * 0.1)
        
        return all_recs.sort_values('Final_Score', ascending=False).head(n_recommendations)
